...
void analyseSteps(const char *filename) {
 ...
 for(auto &st : *steps) {
   auto volName = lookup->volidtovolname[st.volId];
   if (volName->find("MetalStack") < volName->size())
     gM.AddPoint(st.x, st.y);
   if (volName->find("ITSUSensor") < volName->size())
     gS.AddPoint(st.x, st.y);
   if (volName->find("ITSUChip") < volName->size())
     gC.AddPoint(st.x, st.y);
 ...
 //}
  TFile out("stepOut.root", "recreate");
  out.cd();

  gM.SetMarkerColor(kRed);
  gS.SetMarkerColor(kGreen);
  gC.SetMarkerColor(kBlue);

  gS.Write("graph_sensor");
  gM.Write("graph_metal");
  gC.Write("graph_chip");
